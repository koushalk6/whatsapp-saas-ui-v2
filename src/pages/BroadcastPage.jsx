...
export default function BroadcastPage(){
  ...
  async function handleSend(e){
    ...
    if(!templateName || to.length===0){
      setError("Template name and at least one number required.");
      return;
    }
  ...
  return (
    <div>
      ...
      <form onSubmit={handleSend} className="bg-white rounded-lg card-border shadow-sm p-4 space-y-3 mb-4">
        ...
        <div>
          <label className="text-xs text-slate-500">Numbers (comma or newline separated)</label>
          <textarea 
            className="mt-1 w-full border border-slate-200 rounded-md px-3 py-2 text-xs h-24 focus:outline-none focus:ring-1 focus:ring-emerald-500" 
            value={numbers}
